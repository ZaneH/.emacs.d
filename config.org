...
** +tree-sitter
This configuration file is used to add language support to TS and to activate lsp-mode when appropriate. Check that the
versions are up-to-date every so often.
